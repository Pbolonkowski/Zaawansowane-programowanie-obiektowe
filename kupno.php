...
else
{
    ...
    while ($i < $num) {
        ...
        $id_sprz = $row['id_sprzedajacego'];

        $uzytkownik = "SELECT * FROM uzytkownicy WHERE id = $id_sprz";
        $result1=$polaczenie->query($uzytkownik);
        $row1 = $result1->fetch_assoc();

        if(($row["id_kupujacego"] == $_SESSION['id']) && ($row["data_sprzedazy"] != '0000-00-00 00:00:00'))
        {
        #$row1["id"] -> id uzytkownika do ktorego nalezy wyslac maila, jest to sprzedajacy
        ...
        echo "<br>Nick sprzedającego: ".$row1["user"];
        echo "<br>E-mail sprzedającego: ".$row1["email"];
