...
else
{
    ...
    while ($i < $num) {
        ...
        $id_kup = $row['id_kupujacego'];

        $uzytkownik = "SELECT * FROM uzytkownicy WHERE id = $id_kup";
        $result1=$polaczenie->query($uzytkownik);
        $row1 = $result1->fetch_assoc();

        if(($row["id_sprzedajacego"] == $_SESSION['id']) && ($row["data_sprzedazy"] != '0000-00-00 00:00:00'))
        {
        #$row1["id"] -> id uzytkownika do ktorego nalezy wyslac maila, jest to kupujacy
        ...
        echo "<br>Nick kupującego: ".$row1["user"];
        echo "<br>E-mail kupującego: ".$row1["email"];
        echo "<br>Data dodania ogłoszenia: ".$row["data_dodania"];
        echo "<br>Data zakończenia ogłoszenia: ".$row["data_sprzedazy"];
        echo "<br>Kategoria: ".$row["kategoria"];
        echo "<br>";
        echo "</form>";
        echo '<a href="kontakt.php?zmienna='.$row1["id"].'"><button class="button button1">WYŚLIJ WIADOMOŚĆ</button></a>';
        echo "</div>";
        echo "<br><br>";   
        echo "<br><br>";      
