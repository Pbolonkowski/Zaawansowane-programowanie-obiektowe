  ...
  echo '<p class="testowy"> Witaj '.$_SESSION['user'].'! </p>';
    ...
    <!-- <button name="zaloguj">Zaloguj</button> -->
        ...
        <ul class="menu">
          ...
          <li><a href="przegladaj_2.php" class="active">przeglądaj</a></li>
...
else
{
    ...
    while ($i < $num) {
        ...
        if($row["id_kupujacego"] == -2)
        {
        echo '<div id="oferta">'; 
        echo "<form action='kup.php'  method='post' novalidate>";
        echo "<img src='".$row["obraz"]."' id='test_zdjecia'>";
        echo "<b>".$row["nazwa"]."</b>";
        ...
        echo "<br>"; 
//echo '<button class="button button1" name="buttonn" type="sumbit" value='.$row["ID_produktu"].'>KUP PRZEDMIOT</button>';
//echo '<a href="kontakt.php?zmienna='.$row["id_sprzedajacego"].'"><button class="button button1">WYŚLIJ WIADOMOŚĆ</button></a>';

         echo "<div id='przyciskii'><button class='button button1' name='buttonn' type='submit' value='".$row['ID_produktu']."'>KUP PRZEDMIOT</button> ";
//
         echo '<a class="button button11" href="kontakt.php?zmienna='.$row["id_sprzedajacego"].'">WYŚLIJ WIADOMOŚĆ</a>';
        echo "</div>";
        echo "</form>";
        echo "</div>";
